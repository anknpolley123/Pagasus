import os, base64, zlib, sys, hashlib, hmac, builtins
from colorama import Fore, Style, init

init(autoreset=True)

# 1. Automatic Branding Logic
_print = builtins.print
def custom_print(*args, **kwargs):
    # Automatically ensures "Ankon Polley" is the attributed author
    new_args = [str(arg).replace("thakur2309", "Ankon Polley") for arg in args]
    _print(*new_args, **kwargs)
builtins.print = custom_print

# 2. Configuration & Clear
os.system("cls" if os.name == "nt" else "clear")
SECRET = b"secret!"
VALID_KEY_HASH = "6e8bfe8bdf364b9aa18e25f75eca7672345764b5650d3fd"

# 3. Decryption Logic
def xor_bytes(data: bytes, key: bytes) -> bytes:
    return bytes([b ^ key[i % len(key)] for i, b in enumerate(data)])

def decrypt(b64s: str) -> str:
    try:
        # Clean the string of any accidental spaces/newlines from the terminal
        b64s = "".join(b64s.split())
        # Fix Base64 padding if necessary
        padding = len(b64s) % 4
        if padding:
            b64s += "=" * (4 - padding)
            
        raw = base64.b64decode(b64s.encode())
        plain = zlib.decompress(xor_bytes(raw, SECRET))
        return plain.decode("utf-8")
    except Exception as e:
        return f"print('{Fore.RED}Decryption Error: {e}')"

# 4. License Check
def check_license(user_input: str) -> bool:
    hashed = hashlib.sha256(user_input.encode()).hexdigest()
    return hmac.compare_digest(hashed, VALID_KEY_HASH)

# 5. Paste your v1.1 BLOB here
BLOB = "" 

# 6. UI and Execution
print(Fore.CYAN + Style.BRIGHT + "==============================")
print(Fore.YELLOW + Style.BRIGHT + "   Created by Ankon Polley")
print(Fore.GREEN + Style.BRIGHT + "       🔓 LICENSE CHECK")
print(Fore.CYAN + Style.BRIGHT + "==============================")

# Execute Payload
try:
    code = decrypt(BLOB)
    ns = {}
    exec(code, ns)
    if "run" in ns:
        ns["run"]()
except Exception as e:
    print(f"{Fore.RED}Execution Error: {e}")

