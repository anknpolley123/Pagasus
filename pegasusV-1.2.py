import os, base64, zlib, sys, hashlib, hmac, builtins
...
def custom_print(*args, **kwargs):
    # Automatically updates the branding to your name
    new_args = [str(arg).replace("thakur2309", "Ankon Polley") for arg in args]
...
# 3. Secure Decryption & Decompression
def decrypt(b64s):
    try:
        # Clean string of any terminal-added spaces or newlines
        ...
        # XOR Decryption
        decrypted_bytes = bytes([b ^ SECRET[i % len(SECRET)] for i, b in enumerate(raw)])
        
        # Decompress
        return zlib.decompress(decrypted_bytes).decode("utf-8")
    except Exception as e:
        return f"print('{Fore.RED}Error: {e}')"

# 4. Corrected BLOB (Ensure this ends with a quote mark ")
...
# 5. Header and Execution
print(Fore.CYAN + Style.BRIGHT + "==============================")
print(Fore.YELLOW + Style.BRIGHT + "   Created by Ankon Polley")
